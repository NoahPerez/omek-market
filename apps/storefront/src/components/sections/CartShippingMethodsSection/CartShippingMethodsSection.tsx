...
const CartShippingMethodsSection: FC<ShippingProps> = ({ cart, availableShippingMethods }) => {
  ...
  const sellerNameById = new Map<string, string | undefined>(
    (cart.items ?? [])
      .map((item: any) => [item.offer?.seller?.id, item.offer?.seller?.name] as const)
  ...
  const sellerNameByOptionId = new Map<string, string | undefined>(
    (_shippingMethods ?? []).map((option: any) => [
      option.id,
      option.seller_name ?? (option.seller_id ? sellerNameById.get(option.seller_id) : undefined)
    ])
  );
                  ...
                  : filteredGroupedBySellerId.map(key => {
                      ...
                      >
                        <Heading
                          ...
                          className="mb-2"
                        >
                          {groupedBySellerId[key][0].seller_name ?? sellerNameById.get(key) ?? key}
                        </Heading>
                        <Listbox
